  ...
  .poll-common-closing-at-field.my-3
    .poll-common-closing-at-field__inputs
      v-layout(wrap)
        v-flex
          v-menu(ref='menu' v-model='isShowingDatePicker' :close-on-content-click='false' offset-y min-width="290px")
            template(v-slot:activator='{ on, attrs }')
              v-text-field(:disabled="!poll.closingAt" v-model='closingDate' :rules="[validDate]" placeholder="2000-12-30" v-on='on' v-bind="attrs" :prepend-icon="$icons['mdi-calendar']")
        ...
        v-spacer
        v-select.poll-common-closing-at-field__timepicker(:disabled="!poll.closingAt" :prepend-icon="$icons['mdi-clock-outline']" v-model='closingHour' :label="$t('poll_meeting_time_field.closing_hour')" :items="times")
