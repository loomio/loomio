...
async function loadGroup() {
  ...
  try {
    ...
    // Cached data is useful while authorization is pending, but must stop
    // rendering as soon as the server says this group is no longer accessible.
    if ([403, 404].includes(error.status)) group.value = null;
