class MotionMailer < BaseMailer
  ...
  def motion_outcome_created(motion, user)
    ...
    I18n.with_locale(locale) do
      mail  to: user.email,
            from: "#{user.name} <noreply@loomio.org>", #hold till migration#  "#{motion.outcome_author.outcome_author.name} <noreply@loomio.org>",
            #dito, hold#  reply_to: motion.outcome_author.name_and_email,
