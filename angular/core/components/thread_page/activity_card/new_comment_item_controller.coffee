angular.module('loomioApp').controller 'NewCommentItemController', ($scope, $rootScope, $translate, Records, User, ModalService, EditCommentForm, DeleteCommentForm, AbilityService, TranslationService, RevisionHistoryModal) ->
  renderLikedBySentence = ->
    otherIds = _.without($scope.comment.likerIds, User.current().id)
  ...
  $scope.like = ->
    $scope.addLiker()
    Records.comments.like(User, $scope.comment).then (->), $scope.removeLiker

  $scope.unlike = ->
    $scope.removeLiker()
    Records.comments.unlike(User, $scope.comment).then (->), $scope.addLiker

  $scope.currentUserLikesIt = ->
    _.contains($scope.comment.likerIds, User.current().id)
  ...
  $scope.addLiker = ->
    $scope.comment.addLiker(User.current())
    renderLikedBySentence()

  $scope.removeLiker = ->
    $scope.comment.removeLiker(User.current())
    renderLikedBySentence()
