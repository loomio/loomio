...
describe Api::ProfileController do
  ...
  describe 'show' do
    it 'returns the user json' do
      get :show, params: { id: another_user.id, format: :json }
    ...
    it 'does not return a deactivated user' do
      ...
      expect(JSON.parse(response.body)['exception']).to eq 'ActiveRecord::RecordNotFound'
    end
  end
  ...
  describe 'change_password' do
    context 'success' do
      it "changes a users password" do
        ...
        post :change_password, params: { user: { current_password: 'complex_password', password: 'new_password', password_confirmation: 'new_password'}, format: :json }
        expect(response).to be_success
    ...
    context 'failures' do
      it 'does not allow a change if current password does not match' do
        old_password = user.encrypted_password
        post :change_password, params: { user: { current_password: 'not right', password: 'new_password', password_confirmation: 'new_password'}, format: :json }
      ...
      it 'does not allow a change if passord confirmation doesnt match' do
        old_password = user.encrypted_password
        post :change_password, params: { user: { password: 'new_password', password_confirmation: 'errwhoops'}, format: :json }
        expect(response).to_not be_success
        expect(user.reload.encrypted_password).to eq old_password
      end
    end
