class Api::MotionsController < Api::RestfulController
  ...
  def create_outcome
    @event = service.create_outcome(motion: fetch_and_authorize(:motion, :create_outcome),
  ...
  def update_outcome
    @event = service.update_outcome(motion: fetch_and_authorize(:motion, :update_outcome),
                                    params: permitted_params.motion,
                                    actor:  current_user)
    respond_with_resource
  end
  ...
  def index
    instantiate_collection do |collection|
      collection = collection.where(discussion: @discussion) if fetch_and_authorize(:discussion, optional: true)
  ...
  def closed
    instantiate_collection do |collection|
      collection.closed.where(discussion_id: fetch_and_authorize(:group).discussion_ids).order(closed_at: :desc)
    end
