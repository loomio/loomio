module CurrentUserHelper
  ...
  def sign_in(user)
    ...
    start_new_session_for(user) && handle_pending_actions(user) && associate_user_to_visit
  end

  def current_user
    @current_user || resume_session&.user || LoggedOutUser.new(locale: logged_out_preferred_locale, params: params, session: session)
  end
