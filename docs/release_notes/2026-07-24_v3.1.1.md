...
When the inbound relay reports a DMARC failure, or reports both DKIM and SPF failures, Loomio now treats the message as coming from an unrecognized sender instead of publishing it as that member. Operators using a custom inbound relay should configure it to replace or rename sender-supplied `Authentication-Results` headers and stamp its own trusted results. Relays that do not provide authentication results continue to behave as before.

The bundled Haraka relay replaces sender-supplied authentication results and evaluates SPF, DKIM, and DMARC before forwarding mail to Loomio. Authentication failures reach Loomio so it can refuse to attribute a spoofed message to a member without rejecting legitimate forwarded mail that fails SPF but passes aligned DKIM.
...
3. If inbound email uses a custom relay, confirm it replaces or renames sender-supplied `Authentication-Results` headers and stamps its own trusted results.
