angular.module('loomioApp').controller 'RootController', ($scope, $timeout, $location, $router, $mdMedia, AuthModal, KeyEventService, MessageChannelService, IntercomService, ScrollService, Session, AppConfig, Records, ModalService, SignInForm, GroupForm, AbilityService, AhoyService, ViewportService, HotkeyService) ->
  ...
  $scope.$on 'currentComponent', (event, options = {}) ->
    ...
    if !options.skipLogin and (AbilityService.requireLoginFor(options.page) or $location.search().invitation_email)
