...
describe ProposalsClosingSoonJob do
  ...
  let(:unsaved_motion) { build :motion, author: user, group: group, closing_at: 24.hours.from_now }
  ...
  describe 'perform' do
    ...
    it 'does not publish multiple motion_closing_soon events' do
      ...
      expect { ProposalsClosingSoonJob.new.perform }.to_not change { motion.author.notifications.count }
      Timecop.travel(1.day.from_now)

      expect { ProposalsClosingSoonJob.new.perform }.to change { motion.author.notifications.count }.by(1)
      MotionService.update(motion: motion, params: { closing_at: 4.days.from_now.to_s }, actor: motion.author)
      expect { ProposalsClosingSoonJob.new.perform }.to_not change { motion.author.notifications.count }
