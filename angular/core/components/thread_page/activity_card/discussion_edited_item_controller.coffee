angular.module('loomioApp').controller 'DiscussionEditedItemController', ($scope) ->
  ...
  $scope.onlyPrivacyEdited = ->
    ...
    (!version.attributeEdited('title') and !version.attributeEdited('description'))

  $scope.privacyKey = ->
    return unless version.changes.private? and
                  (version.changes.private[0] != version.changes.private[1])
