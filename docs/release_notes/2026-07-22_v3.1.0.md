...
A separate [security advisory](ADD_LINK_BEFORE_RELEASE) provides further technical details.
...
[Read more about poll comments on Loomio Community](ADD_LINK_BEFORE_RELEASE).
...
7. **Session handling has changed.** Existing users remain signed in, but operators should review any custom authentication, session, OAuth, or SAML integrations.
