...
describe UsersController do
  ...
  describe "#upload_new_avatar" do
    before do
      @user = create(:user)
      controller.allow(:current_user).and_return(@user)
      controller.allow(:get_notifications)
      @user.allow(:avatar_kind=).with("uploaded")
      @user.allow(:uploaded_avatar=).with("www.peter_chilltooth.jpg")
    end
