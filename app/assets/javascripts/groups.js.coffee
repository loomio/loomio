...
$ ->
  if $("body.groups.add_subgroup").length > 0 || $("body.groups.edit").length > 0
    toggle_visible_to_parent_members_ability()
...
toggle_visible_to_parent_members_ability = ->
  if $('input[name="group[privacy]"]:checked').val() == 'public'
    $('.group_visible_to_parent_members label').addClass('disabled')
    $('input#group_visible_to_parent_members').attr('disabled', true)
  else
    $('.group_visible_to_parent_members label').removeClass('disabled')
    $('input#group_visible_to_parent_members').removeAttr('disabled')
