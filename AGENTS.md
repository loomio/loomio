...
- Write concise commit messages that explain what changed and why it was necessary. Use one or two sentences for each distinct fix; issue IDs are optional context and must not replace the explanation.
- When fixing multiple issues, make small, specific commits rather than combining unrelated fixes. Keep each commit independently coherent and include its relevant tests.
...
- Use and curate the reusable access-control fixture matrix rooted at `topics(:discussion_topic)` and `topics(:direct_topic)`. Its named roles represent current members, topic guests, members of `alien_group`, non-guest readers, revoked access, inactive users, and conflicting account/membership/topic preferences. Extend the fixture matrix whenever a new access boundary or preference source is introduced; do not replace matrix coverage with isolated ad hoc records.
...
- Whenever editing a `.vue` template line that uses `v-t`, replace the directive with `{{ t(...) }}` interpolation and obtain `t` from `useI18n()`.
