...
module EmptyTrialCleanupService
  ...
  def self.candidate_trees(expires_before:, root_id: nil)
    trial_trees(expires_before: expires_before, root_id: root_id, topic_free: true)
  end

  # Returns expired trial trees that pass billing and subscription-link safety
  # checks, whether or not they contain content.
  def self.eligible_trees(expires_before:, root_id: nil)
    trial_trees(expires_before: expires_before, root_id: root_id, topic_free: false)
  end

  def self.trial_trees(expires_before:, root_id:, topic_free:)
    sql = Group.sanitize_sql_array([ <<~SQL, { expires_before: expires_before, root_id: root_id, topic_free: topic_free } ])
  ...
  private_class_method :trial_trees, :record_counts, :log
end
