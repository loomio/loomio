...
class GroupExportServiceTest < ActiveSupport::TestCase
  def next_sequence_value(model)
    sequence_name = ActiveRecord::Base.connection.default_sequence_name(model.table_name, model.primary_key)
    row = ActiveRecord::Base.connection.select_one("SELECT last_value, is_called FROM #{ActiveRecord::Base.connection.quote_table_name(sequence_name)}")
  ...
  test "import translates topicable ids before insert without colliding with old imported topicable ids" do
    next_discussion_id = next_sequence_value(Discussion)
    old_first_discussion_id = next_discussion_id + 10_000
    old_second_discussion_id = next_discussion_id
    old_first_topic_id = next_sequence_value(Topic) + 20_000
    old_second_topic_id = old_first_topic_id + 1
    ...
    records = [
      {
        table: 'discussions',
        record: {
          id: old_first_discussion_id,
          title: 'first imported discussion',
          description: '',
          topic_id: old_first_topic_id,
          created_at: now,
          updated_at: now
        }
      },
      {
        table: 'discussions',
        record: {
          id: old_second_discussion_id,
          title: 'second imported discussion',
          description: '',
          topic_id: old_second_topic_id,
          created_at: now,
          updated_at: now
        }
      },
      {
        table: 'topics',
        record: {
          id: old_first_topic_id,
          topicable_type: 'Discussion',
          topicable_id: old_first_discussion_id,
          created_at: now,
          updated_at: now
        }
      },
      {
        table: 'topics',
        record: {
          id: old_second_topic_id,
          topicable_type: 'Discussion',
          topicable_id: old_second_discussion_id,
          created_at: now,
          updated_at: now
        }
      }
    ...
    file = Tempfile.new(['group-export-topicable-collision', '.json'])
    records.each { |record| file.puts(record.to_json) }
    file.close

    GroupExportService.import(file.path)

    first_discussion = Discussion.find_by!(title: 'first imported discussion')
    second_discussion = Discussion.find_by!(title: 'second imported discussion')

    assert_equal first_discussion.id, first_discussion.topic.topicable_id
    assert_equal second_discussion.id, second_discussion.topic.topicable_id
  ensure
    file&.unlink
  end

  test "import maps exported users to existing users by email before inserting records" do
    existing_user = User.create!(email: "existing-export-user#{SecureRandom.hex(4)}@example.com", name: 'existing', password: 'password', email_verified: true)
    old_user_id = existing_user.id + 10_000
    old_discussion_id = next_sequence_value(Discussion) + 10_000
    old_topic_id = next_sequence_value(Topic) + 10_000
    now = Time.current.iso8601
    ...
    records = [
      ...
      },
      {
        table: 'discussions',
        record: {
          id: old_discussion_id,
          author_id: old_user_id,
          title: 'existing user imported discussion',
          description: '',
      ...
      },
      {
        ...
        record: {
          id: old_topic_id,
          topicable_type: 'Discussion',
          topicable_id: old_discussion_id,
          created_at: now,
          updated_at: now
        }
      }
    ...
    file = Tempfile.new(['group-export-existing-user', '.json'])
    records.each { |record| file.puts(record.to_json) }
    file.close

    assert_no_difference -> { User.where(email: existing_user.email).count } do
      GroupExportService.import(file.path)
    end

    discussion = Discussion.find_by!(title: 'existing user imported discussion')
    assert_equal existing_user.id, discussion.author_id
  ensure
    file&.unlink
  end
end
