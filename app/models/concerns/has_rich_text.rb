    ...
    def is_rich_text(on: [])
      ...
      rich_text_fields.each do |field|
        define_method "sanitize_#{field}!" do
          ...
          attributes = %w[href src alt title class data-type data-done data-mention-id data-author-id data-uid data-checked data-due-on width height target colspan rowspan data-text-align background style]
