...
describe DiscussionsController do
  ...
  context "authenticated user" do
    before do
      ...
      allow(app_controller).to receive(:cannot?).with(:show, group).and_return(false)
      ...
      allow(Group).to receive(:find).with(group.key).and_return(group)
    end

    context "deleting a discussion" do
      before do
        allow(discussion).to receive(:delayed_destroy)
        # allow(controller).to receive(:authorize!).with(:destroy, discussion).and_return(true)
      end
    ...
    describe "creating a new proposal" do
      ...
      context "where no current proposal exists" do
        before do
          allow(discussion).to receive(current_motion: nil)
          # allow(Discussion).to receive(:find).with(discussion.id.to_s).and_return(discussion)
          get :new_proposal, id: discussion.key
        end
        it "succeeds" do
    ...
    describe "add_comment" do
      ...
      before do
        ...
        allow(DiscussionService).to receive(:add_comment)
        allow(Event).to receive(:new_comment!)
    ...
    describe "change version" do
      before do
        ...
        PaperTrail::allow(Version).to receive(:find).and_return(@version)
        @allow(version).to receive(:reify).and_return(@version_item)
