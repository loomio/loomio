class API::AnnouncementsController < API::RestfulController
  ...
  def history
    ...
    events = Event.where(kind: ['announcement_created', 'user_mentioned', 'announcement_resend', 'discussion_announced', 'poll_announced', 'outcome_announced', 'comment_replied_to'], eventable: history_model).order('id').limit(50)
