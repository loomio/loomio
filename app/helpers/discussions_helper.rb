module DiscussionsHelper
  def activity_count_for_discussion(discussion, user)
    user ? user.discussion_activity_count(discussion) : 0
  end

  def enabled_icon_class_for_discussion(discussion, user)
    if activity_count_for_discussion(discussion, user) > 0
      "enabled-icon"
    else
      "disabled-icon"
    end
