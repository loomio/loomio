angular.module('loomioApp').controller 'AdminMembershipsPanelController', ($scope, User, AbilityService, ModalService, Records, FlashService, RemoveMembershipForm, InvitationForm, $filter) ->
  ...
  $scope.toggleAdmin = (membership) ->
    ...
    if !membership.admin and membership.user() == User.current() and !confirm($filter('translate')('memberships_page.remove_admin_from_self.question'))
