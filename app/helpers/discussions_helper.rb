module DiscussionsHelper
  ...
  def css_class_unread_discussion_activity_for(group_page, discussion, user)
    ...
    css_class += " showing-group" if (not discussion.group.parent.nil?) && (group_page && (group_page.parent.nil?))
    css_class += " unread" if user_signed_in? && discussion.unread_by(user)
    css_class
  end

  def css_class_unread_group_activity_for(group_page, discussion, user)
    css_class = "group-activity-indicator"
    css_class += " unread-group-activity" if (user_signed_in? && (not group_page.nil?) && discussion.has_activity_since_group_last_viewed?(user))
    # if user_signed_in? && params[:group_id] && discussion.has_activity_since_group_last_viewed?(user)
    css_class
  end
