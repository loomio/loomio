class MergeUsersController < ApplicationController
  def confirm
    ...
    if MergeUsersService.validate(source_user: source_user, target_user: target_user, hash: hash)
      render Views::Web::MergeUsers::Confirm.new(source_user: source_user, target_user: target_user, hash: hash)
    else
  ...
  def merge
    ...
    if MergeUsersService.validate(source_user: source_user, target_user: target_user, hash: hash)
      ...
      render Views::Web::MergeUsers::Complete.new(target_user: target_user)
    else
      respond_with_error 422
    end
  end
