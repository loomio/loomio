angular.module('loomioApp').directive 'proposalPositionsPanel', ->
  ...
  replace: true
  controller: ($scope, Records, User, ModalService, VoteForm, AbilityService) ->

    $scope.undecidedPanelOpen = false

    $scope.changeVote = ->
      ModalService.open VoteForm, vote: -> $scope.proposal.lastVoteByUser(User.current()).clone()
