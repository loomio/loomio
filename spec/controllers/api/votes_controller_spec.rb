...
describe API::VotesController do
  ...
  let(:other_guys_vote)       { create :vote, motion: motion,         user: another_user }
  let(:vote_params) {{
    position: 'yes',
  ...
  describe 'create' do
    context 'success' do
      it "creates a vote" do
        post :create, vote: vote_params, format: :json
    ...
    context 'failures' do
      it "responds with an error when there are unpermitted params" do
        ...
        expect { post :create, vote: vote_params, format: :json }.to raise_error 
      end
