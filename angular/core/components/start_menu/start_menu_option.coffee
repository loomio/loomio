angular.module('loomioApp').directive 'startMenuOption', ->
  ...
  replace: true,
  controller: ($scope, ModalService, InvitationForm, DiscussionForm, GroupForm, Records, AbilityService, KeyEventService, Session) ->
    ...
    $scope.openModal = ->
      switch $scope.action
        ...
        when 'startThread'
          ModalService.open DiscussionForm, discussion: -> Records.discussions.build(groupId: Session.currentGroupId())
    if $scope.hotkey
