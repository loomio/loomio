angular.module('loomioApp').factory 'ThreadQueryService', (Records, AbilityService, User) ->
    ...
    applyFilters = (view, filters, queryType) ->
      ...
      _.each filters, (filter) ->
        switch filter
          ...
          when 'show_not_muted'
            view.applyWhere (thread) -> thread.volume() != 'mute'
          when 'only_threads_in_my_groups' then view.applyFind(groupId: {$in: User.current().groupIds()})
