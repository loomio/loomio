...
class GroupMentionedNotificationTest < ActiveSupport::TestCase
  inline_jobs
  setup do
  ...
  test "does not notify the actor when they mention the group" do
    ...
    assert_not NotificationDelivery.joins(:notification).exists?(
      recipient: @actor,
      notifications: { kind: "group_mentioned" }
    )
  end
  ...
  test "notifies normally" do
    comment = Comment.new(parent: @discussion, author: @actor,
                          body: "hello @#{@group.handle}", body_format: 'md')
    assert_difference -> { Notification.where(kind: "group_mentioned").count }, 1 do
      publish_new_comment_with_mentions(comment)
    end
  end
  ...
  test "does not notify if members cannot announce" do
    ...
    comment = Comment.new(parent: @discussion, author: non_admin,
                          body: "hello @#{@group.handle}", body_format: 'md')
    assert_no_difference -> { Notification.where(kind: "group_mentioned").count } do
      publish_new_comment_with_mentions(comment)
    end
  end
  ...
  test "notifies group on edit with newly mentioned group" do
    ...
    publish_new_comment_with_mentions(comment)
    assert_equal 0, Notification.where(kind: "group_mentioned").count

    comment.update!(body: "edited to mention group @#{@group.handle}")
    publish_mentions(comment)
    assert_equal 1, Notification.where(kind: "group_mentioned").count
  end
