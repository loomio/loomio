...
```bash
docker pull loomio/loomio:latest
```
...
```bash
docker build -t loomio/loomio:local .
```
...
```bash
docker run --rm loomio/loomio:latest uname -m
```
...
2. Build for a single platform using `--load`:
   ```bash
   docker buildx build --platform linux/amd64 --load -t loomio/loomio:local .
   ```
