...
.poll-meeting-form-options
  v-menu(ref="menu" v-model="menu" :close-on-content-click="false" offset-y full-width min-width="290px")
    template(v-slot:activator="{ on }" )
      v-combobox.poll-meeting-time-field__datepicker-container(v-on="on" v-model="poll.pollOptionNames" @change="persistOptions()" multiple chips small-chips deletable-chips readonly :label="$t('poll_meeting_form.timeslots')")
