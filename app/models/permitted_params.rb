class PermittedParams < Struct.new(:params)
  ...
  def group_attributes
    [:parent_id, :name, :group_privacy, :is_visible_to_public, :discussion_privacy_options,
     ...
     :description, :is_visible_to_parent_members, :parent_members_can_see_discussions, :members_can_start_discussions,
     :members_can_create_subgroups, :membership_granted_upon, :cover_photo, :logo, :category_id, :members_can_raise_motions,
     :members_can_vote, :document_ids, {document_ids: []}, :features, features: AppConfig.group_features
   ]
  end

  def announcement_attributes
    [:announceable_id, :announceable_type,
     :notified, {notified: [:id, :icon_url, :title, :subtitle, :type, :notified_ids, {notified_ids: []}]}]
  end
