...
class Views::Email::Poll::ResultsPanel < Views::Email::Base
  ...
  def view_template
      ...
      div(class: "poll-mailer__results-chart poll-mailer-common-results") do
        if @poll.show_results?(voted: my_stance && my_stance.cast_at)
          ...
          if @poll.poll_type == "meeting"
            render Views::Email::Poll::Results::Meeting.new(poll: @poll, recipient: @current_user)
          else
            render Views::Email::Poll::Results::Simple.new(poll: @poll, recipient: @current_user)
          end
        else
    ...
    render Views::Email::Poll::Undecided.new(poll: @poll)
  end
end
