module HasRichText
  ...
  def build_attachments
    ...
    self[:attachments] = files.map do |file|
      ...
      i.merge!({ preview_url: Rails.application.routes.url_helpers.rails_representation_url(file.representation(PREVIEW_OPTIONS)) }) if file.representable?
      i.merge!({ download_url: Rails.application.routes.url_helpers.rails_blob_url(file, disposition: "attachment") })
