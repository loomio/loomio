...
v-card.auth-form(:title="$t('auth_form.sign_up_or_log_in', { site_name: siteName })")
  ...
  v-sheet
    ...
    p.text-h6.text-center(v-if="pendingGroup" v-t="{path: 'auth_form.youre_invited', args: {group_name: pendingGroup.name}}")
    p.text-h6.text-center(v-if="pendingDiscussion" v-t="'auth_form.youre_invited_discussion'")
    p.text-h6.text-center(v-if="pendingPoll" v-t="'auth_form.youre_invited_poll'")
