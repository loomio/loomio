angular.module('loomioApp').directive 'groupActionsDropdown', ->
  ...
  controller: ($scope, $window, AppConfig, AbilityService, User, ChangeVolumeForm, ModalService, GroupForm, LeaveGroupForm, ArchiveGroupForm, Records, ChoosePlanModal) ->
    ...
    @openChangeVolumeForm = ->
      ModalService.open ChangeVolumeForm, model: -> $scope.group.membershipFor(User.current())
