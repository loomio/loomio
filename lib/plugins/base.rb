module Plugins
  ...
  class Base
    ...
    def use_e2e(path)
      @actions.add Proc.new {
        puts "Copying from #{path_prefix(path)} to #{Rails.root}/client/angular/test/nightwatch/plugins/"
        system("cp #{path_prefix(path)} #{Rails.root}/client/angular/test/nightwatch/plugins/")
      }.to_proc
    end
