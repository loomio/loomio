...
module.exports = {
  'can sign up a user': (test) => {
    ...
    page.loadPath('setup_dashboard_as_visitor', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'max_von_sydow@example.com')
    page.click('.auth-email-form__submit', '.auth-signup-form__name input')
  ...
  'can sign up a new user through the discussion page': (test) => {
    ...
    page.loadPath('view_open_discussion_as_visitor', '.add-comment-panel__sign-in-btn',)
    page.click('.add-comment-panel__sign-in-btn', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'max_von_sydow@example.com')
    page.click('.auth-email-form__submit', '.auth-signup-form__name input')
    page.fillIn('.auth-signup-form__name input', 'Max Von Sydow')
    page.click('.auth-signup-form__submit', '.auth-complete')
    ...
    page.waitFor('.add-comment-panel__join-actions button')
    page.click('.add-comment-panel__join-actions button', '.flash-root__message')
    page.expectText('.flash-root__message', 'You are now a member of Open Dirty Dancing Shoes')
    page.waitFor('.comment-form__submit-button')
  ...
  'can login via token': (test) => {
    ...
    page.loadPath('setup_login_token', '.auth-signin-form__submit')
    page.click('.auth-signin-form__submit', '.flash-root__message')
    page.expectText('.flash-root__message', 'Signed in successfully')
  },
  ...
  'can use a shareable link': (test) => {
    ...
    page.loadPath('view_closed_group_with_shareable_link', '.auth-email-form__email input',)
    page.fillIn('.auth-email-form__email input', 'max_von_sydow@example.com')
    page.click('.auth-email-form__submit', '.auth-signup-form__name input')
    page.fillIn('.auth-signup-form__name input', 'Max Von Sydow')
    page.click('.auth-signup-form__submit', '.auth-complete')
  ...
  'does not log in when password is incorrect': (test) => {
    ...
    page.loadPath('setup_explore_as_visitor', '.navbar__sign-in')
    page.click('.navbar__sign-in', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'patrick_swayze@example.com')
    page.click('.auth-email-form__submit', '.auth-signin-form__password input')
    page.fillIn('.auth-signin-form__password input', 'w0rstmovie')
    page.click('.auth-signin-form__submit', '.auth-form')
  ...
  'can send login link to user with a password': (test) => {
    ...
    page.loadPath('setup_dashboard_as_visitor', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'patrick_swayze@example.com')
    page.click('.auth-email-form__submit', '.auth-signin-form__login-link')
    page.click('.auth-signin-form__login-link', '.auth-complete')
    page.expectText('.auth-complete', 'Check your email')
    page.expectText('.auth-complete', 'instantly log in')
    page.loadPath('use_last_login_token', '.auth-signin-form__submit')
    page.click('.auth-signin-form__submit', '.flash-root__message')
  ...
  'does not log in an invalid token': (test) => {
    page.loadPath('setup_used_login_token', '.auth-signin-form__submit')
  ...
  'can login from the dashboard': (test) => {
    ...
    page.click('.auth-email-form__submit', '.auth-signin-form__submit')
    page.click('.auth-signin-form__submit', '.auth-complete')
    page.expectText('.auth-complete', 'Check your email')
    page.expectText('.auth-complete', 'instantly log in')
    page.loadPath('use_last_login_token', '.auth-signin-form__submit')
    page.click('.auth-signin-form__submit', '.flash-root__message')
  ...
  'can login from a discussion page': (test) => {
    ...
    page.click('.comment-form__submit-button', '.flash-root__message')
    page.pause(3000)
  ...
  'can login from a closed group page': (test) => {
    ...
    page.loadPath('view_closed_group_as_visitor', '.navbar__sign-in')
    page.click('.navbar__sign-in', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'patrick_swayze@example.com')
    page.click('.auth-email-form__submit', '.auth-signin-form__password input')
    page.fillIn('.auth-signin-form__password input', 'gh0stmovie')
    page.click('.auth-signin-form__submit', '.flash-root__message')
    page.expectText('.flash-root__message', 'Signed in successfully')
    page.waitFor('.group-theme__name')
  ...
  'can login from a secret group page': (test) => {
    ...
    page.loadPath('view_secret_group_as_visitor', '.auth-email-form__email input')
    page.fillIn('.auth-email-form__email input', 'patrick_swayze@example.com')
    page.click('.auth-email-form__submit', '.auth-signin-form__password input')
    page.fillIn('.auth-signin-form__password input', 'gh0stmovie')
    page.click('.auth-signin-form__submit', '.flash-root__message')
  ...
  'can accept an invitation': (test) => {
    ...
    page.loadPath('setup_invitation_to_user_with_password', '.auth-email-form__submit')
    page.click('.auth-email-form__submit', '.auth-signin-form__password input')
    page.fillIn('.auth-signin-form__password input', 'gh0stmovie')
    page.click('.auth-signin-form__submit', '.flash-root__message')
    page.expectText('.flash-root__message', 'Signed in successfully')
  ...
  'can log someone in from an invitation': (test) => {
    ...
    page.loadPath('setup_invitation_to_visitor', '.auth-email-form__submit')
    page.click('.auth-email-form__submit', '.auth-signup-form')
  ...
  'prompts the user to contact us to reactivate': (test) => {
    ...
    page.loadPath('setup_deactivated_user', '.auth-email-form__email input',)
    page.fillIn('.auth-email-form__email input', 'patrick_swayze@example.com')
    page.click('.auth-email-form__submit', '.auth-inactive-form')
    page.expectText('.auth-inactive-form', 'has been deactivated!')
    page.click('.auth-inactive-form__submit', '.contact-form')
