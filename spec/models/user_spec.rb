...
describe User do
  ...
  describe "open_motions" do
    it "returns open motions that belong to user" do
      motion = create(:motion, author: user)
      user.open_motions.should include(motion)
    end

    it "should not return closed motions that belong to the user" do
      motion = create(:motion, author: user)
      motion.close!
      user.open_motions.should_not include(motion)
    end
  end

  describe "closed_motions" do
    it "returns closed motions that belong to the user" do
      motion = create(:motion, author: user)
      motion.close!
      user.closed_motions.should include(motion)
    end

    it "should not return open motions that belong to the user" do
      motion = create(:motion, author: user)
