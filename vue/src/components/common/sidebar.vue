...
v-navigation-drawer.sidenav-left.lmo-no-print(app v-model="open")
  ...
  v-list(nav)
    ...
    v-list-item(v-if="showHelp", :href="helpURL" target="_blank" density="compact" lines="two")
    ...
    v-list-item(density="compact" to="/explore" v-if="showExploreGroups")
      v-list-item-title(v-t="'sidebar.explore_groups'")
      template(v-slot:append)
        common-icon(name="mdi-map-search")
    v-list-item(v-if="showContact" @click="$router.replace('/contact')" density="compact" lines="two")
